...
export function getVisualState(data) {
  ...
  return {
    ...
    style: [
      ...
      `--city-outline-soft:${hexToRgba(cityTheme.accent || theme.accent, 0.14)}`,
      ...
      `--city-motif-stroke:${hexToRgba(cityTheme.accent || theme.accent, 0.22)}`,
      `--city-motif-fill:${hexToRgba(cityTheme.halo || cityTheme.accent || theme.glow, 0.08)}`,
      `--city-motif-opacity:${Number.isFinite(cityVisual?.motif?.opacity) ? cityVisual.motif.opacity : 0.58}`,
      `--city-plate:${hexToRgba(cityTheme.wash || theme.glow, 0.2)}`,
